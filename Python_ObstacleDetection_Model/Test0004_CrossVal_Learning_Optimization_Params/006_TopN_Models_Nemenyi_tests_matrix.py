...
def gerar_matriz_nemenyi(metrica="Accuracy", top_n=20):
    ...
    arquivo_nemenyi = f"nemenyi_results_{metrica_id}.csv"
    arquivo_selecionados = f"modelos_selecionados_{metrica_id}.csv"
    arquivo_saida_csv = f"{metrica_id}_top{top_n}_nemenyi_matrix.csv"
    ...
    # Salva a matriz renomeada como CSV
    matriz_topN_renomeada.to_csv(arquivo_saida_csv)
    print(f"[INFO] Matriz de Nemenyi salva em: {arquivo_saida_csv}")
    ...
    print(f"[INFO] Heatmap salvo como imagem em: {arquivo_saida_fig}")
    return matriz_topN_renomeada

if __name__ == "__main__":
    # Altere aqui conforme a métrica desejada
    # Escolha: "Accuracy", "Precision", "Recall", "F1-Score", "ROC-AUC", ou "Weighted"
    metrica = "Weighted"  # Pode ser alterado para "F1-Score", "Precision", etc.
    top_n = 10
