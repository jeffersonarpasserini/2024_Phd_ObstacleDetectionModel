...
\begin{abstract}
This study investigates the application of convolutional neural networks (CNNs) combined with dimensionality reduction techniques and supervised classifiers to assist visually impaired individuals in obstacle detection. Without requiring fine-tuning or retraining, 22 CNN architectures were used solely as feature extractors. The features were reduced using PCA, UMAP, or Relief-F and classified using models such as SVM and MLP. Among the four proposed approaches, the best configuration achieved 0.9559 accuracy. Heatmap analyses revealed that models often misclassified paths due to floor markings, highlighting challenges in semantic understanding. The proposed methodology, relying only on transfer learning, shows that high-performance systems can be built without retraining, making it feasible for lightweight embedded applications.
\end{abstract}

%%Research highlights
\begin{highlights}
	\item 22 CNN architectures were evaluated for obstacle detection in urban navigation.
	\item Feature extraction used transfer learning without retraining the CNNs.
	\item Dimensionality reduction techniques improved classifier performance.
	\item Best model reached 95.59\% accuracy using EfficientNetB0 and MobileNet features.
	\item The approach is lightweight and suitable for mobile or embedded deployment.
\end{highlights}
