...
\title{Cross-Analysis of CNN Architectures as Feature Extractors for Obstacle Detection to Aid the Visually Impaired}
...
\ead{jefferson.passerini@unesp.br}
...
\begin{abstract}
	This study presents a comparative analysis of convolutional neural networks (CNNs) used as feature extractors, combined with dimensionality reduction techniques and supervised classifiers, for obstacle detection aimed at assisting visually impaired individuals. Without requiring fine-tuning or retraining, 25 CNN architectures -- including EfficientNet variants up to B7 -- were used as feature extractors, while a reference model based on 22 fine-tuned architectures served as a baseline. The features were reduced using PCA, UMAP, or Relief-F and classified using models such as SVM and MLP. Among the four proposed approaches, the best accuracy (95.59\%) was achieved using a combination of MobileNet and ResNet50 features with a linear SVM in the 10-fold cross-validation. However, the most consistent performance across all evaluations, including the LOOCV (94.74\%) and the subset of images with ground traffic signage (92.59\%), was obtained with the EfficientNetB0 + MobileNet configuration. Heatmap analyses revealed challenges in classifying signage-based images, suggesting future improvements using attention mechanisms or semantic segmentation. This offline comparative study shows that CNNs used purely as feature extractors can match a fine-tuned baseline at a fraction of its training cost; the on-device deployment and evaluation of the selected model on a smartphone are left for future work.
\end{abstract}
...
\begin{highlights}
	\item 25 CNN architectures evaluated as feature extractors for obstacle detection in urban navigation.
	...
	\item Best accuracy of 95.59\% achieved with MobileNet and ResNet50 features and a linear SVM.
	\item The most stable configuration across tests was EfficientNetB0 with MobileNet.
	\item The classification stage adds negligible computational cost (about 6.7 ms per image).
\end{highlights}
...
Individuals with visual impairments face numerous challenges in their daily lives, especially in terms of mobility and navigation. Traditionally, this population has relied on canes and guide dogs as their primary means of locomotion \cite{islam2019}. However, advancements in computer vision have led to the development of various navigation systems \cite{islam2019,mandia2023,budrionis2022,kuriakose2021,cardillo2019,bai2019,jiang2019,joe2019,hoang2017,tapu2017,saffoury2016,poggi2016,lakde2015}. It is important to note that many of these systems are limited by the need for expensive, bulky, and/or customized equipment \cite{hoang2017,tapu2017,saffoury2016,poggi2016,poggi2015,rizzo2017}, as well as the requirement for high computational power, rendering them impractical for use on mobile devices. Additionally, some systems necessitate a network connection to a more powerful remote server \cite{jiang2019,lin2017}.
...
Building on the work of Breve and Fisher \cite{breve2020} and Breve \cite{breve2023}, this paper does not seek to train a better classifier, but to systematically assess whether CNNs used \emph{purely as feature extractors} -- without any fine-tuning -- can match a fine-tuned baseline when combined with dimensionality reduction/selection and classical supervised classifiers. To the best of our knowledge, a cross-analysis of this breadth -- spanning individual CNNs, combinations of CNNs, three reduction/selection methods, and eight classifiers -- has not been reported for the obstacle-detection task addressed here. The key contributions are as follows:

1. A systematic cross-analysis of 25 CNN feature extractors -- and their pairwise combinations -- three dimensionality reduction/selection algorithms (PCA, UMAP, Relief-F), and eight supervised classifiers, all evaluated without training any CNN.
2. Evidence that the best configurations are statistically equivalent (Friedman test with Nemenyi post-hoc) to a fine-tuned, 22-architecture reference model, at a fraction of its computational cost.
3. A comparison of four feature-combination strategies (full features, single-CNN reduction, and two concatenation schemes), identifying which of them generalise reliably under leave-one-out validation.
4. An interpretability-based error analysis (Grad-CAM) that reveals a systematic failure mode -- the confusion between obstacles and painted ground signage -- and points to concrete directions for future improvement.
...
\begin{figure}
	\includegraphics[width=\linewidth]{figVia.eps}
...
In addition to the 22 architectures listed in Table~\ref{tab:CNNArch}, which compose the reference model and are also employed as feature extractors, the EfficientNetB5, EfficientNetB6, and EfficientNetB7 variants \cite{tan2019} were used exclusively as feature extractors in the proposed approaches. Because using a CNN as a feature extractor relies solely on transfer learning and does not require fine-tuning, these larger variants could be evaluated in this role despite the hardware constraints that prevented their training as part of the reference model.

For dimensionality reduction, the Principal Component Analysis (PCA) \cite{jolliffe2002} and Uniform Manifold Approximation and Projection (UMAP) \cite{mcinnes2021} algorithms were used. Feature selection was also conducted using the Relief-F selection model \cite{kononenko1994}.
...
The tests of the proposed approaches and reference CNNs were carried out using k-fold cross-validation ($k=10$). To verify the test results, the accuracy (acc) was measured at each execution of the classifier. For statistical validation of the results and ranking of the models within each approach, the non-parametric Friedman test was used, through the statistical software IBM SPSS Statistics, version 20.0.0. As the models were evaluated on the same folds (repeated measures), the Friedman test is appropriate both to verify the existence of statistically significant differences and to rank the models. The complete per-fold results, spreadsheets, and statistical outputs of all experiments are publicly available at \url{https://github.com/jeffersonarpasserini/Phd-Partial-Results}.
%% NOTA AOS AUTORES (revisao): considerar reportar tambem recall/sensibilidade e especificidade, dado o custo assimetrico dos erros (falso negativo = obstaculo nao detectado); e aplicar o pos-teste de Nemenyi ao ranqueamento de Friedman.
...
\begin{figure}
	\centering
	\includegraphics[width=0.50\textwidth]{figCNNaddlayer.pdf}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.42\textwidth]{figFeatExtrProcess.pdf}
	\caption{Example of the feature extraction process (VGG16).} \label{fig:FeatExtrProcess}
\end{figure}

The second stage of the proposed method involves the use of a model for reduction and/or selection of features or the passing through of all extracted features. The feature reduction algorithms applied were Principal Component Analysis (PCA) \cite{jolliffe2002}, Uniform Manifold Approximation and Projection (UMAP) \cite{mcinnes2021}, and the feature selection algorithm used was the Relief-F model \cite{kononenko1994}. In this phase, when the algorithm was applied, feature vectors of different sizes were generated: 2, 10, 20, 30, 40, 50, 75, 100, 150, 200, 250, and 300 features, reducing the original dimensionality of the output from the convolutional neural network (See: Fig.~\ref{fig:ModelsCombination}).

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.62\textwidth]{figModelProposed.pdf}
	\caption{Combination of possible algorithms at each stage.} \label{fig:ModelsCombination}
\end{figure}

In the third stage, classification is performed using the feature sets produced in stage 2, with the classifiers introduced previously.

Fig.~\ref{fig:ModelsCombination} summarizes the resources used for analyzing the proposed model and demonstrates combined approaches of CNN architectures in feature extraction in stage 1 of this model. It can also be concluded that there are many possible combinations among stages 1, 2, and 3, thereby structuring four study approaches that we will denote as A, B, C, and D. Tests were initially conducted within each set of the proposed approaches. Within each group of approaches, the model that achieved the highest statistical relevance for the problem was selected.
...
In Approach A, feature extraction was utilized for all the individual CNNs - Fig.~\ref{fig:ApproachA}a and their combinations – Fig.~\ref{fig:ApproachA}b, using the CNN combinations described in this section, leveraging the full output of features extracted by the convolutional network without the process of dimensionality reduction, and feeding the data into each of the classifiers used. This approach generated 272 different technique combinations.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.60\textwidth]{figApproachA.pdf}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.42\textwidth]{figApproachD.pdf}
	\caption{Proposed methodology in Approach D.} \label{fig:ApproachD}
\end{figure}

\section{Results}

The tests were conducted using k-fold cross-validation (k=10), and the non-parametric Friedman test was performed to determine the best approach for the proposed problem; this test uses the median as a measure of performance.
...
Upon applying the test to the reference model, the Friedman test indicated a statistically significant difference among the architectures examined $[X^2(21)=75.062; p<0.0001]$, with the significance level set at $p<0.05$, leading to the rejection of the null hypothesis and enabling the ranking of the architectures. The descriptive outcomes of these performance assessments, which are based on the accuracy achieved and the classification ranking of each studied CNN architecture, are detailed in Table~\ref{tab:Reference_ranking}.
...
\begin{table}[h!]
	...
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{|l|c|c|c|c|c|}
			...
			EfficientNetB3 & 14.55 & 0.8824 & 0.9118 & 0.9210 & 0.0386 \\
...
For the proposed Approach A, the tests followed the same standards, wherein the Friedman test made it possible to verify that there is a statistically significant difference between the techniques studied $[X^2(271)=1,731.191; p<0.0001]$. The significance level of $p<0.05$ determines the rejection of the null hypothesis. In Table~\ref{tab:Approach_A_ranking}, one can observe the descriptive results of the performance tests and the corresponding Friedman ranking. Due to the large number of combinations of techniques, the twenty highest-ranked techniques were presented.
...
The proposed model described in Approach B utilized individual CNNs combined with dimensionality reduction models that resulted in output vectors of sizes 2, 10, 20, 30, 40, 50, 75, 100, 150, 200, 250, and 300. These were then applied to 8 studied classifiers, resulting in a considerable number of technique combinations (7200). Thus, the Friedman test was applied to each of the CNNs individually, resulting in 288 technique combinations for each, thereby selecting the best-performing proposal for each CNN.

From this initial selection within Approach B, the most relevant technique combinations for this approach were analyzed. The Friedman test yielded $[X^2(24)=49.546; p<0.0001]$, confirming statistically significant differences between the employed techniques and enabling the ranking shown in Table~\ref{tab:Approach_B_ranking}, which identified the best technique within Approach B in this study.
...
From this initial selection within Approach C, the most relevant technique combinations for this approach were analyzed. The Friedman test resulted in $[X^2(8)=22.989; p<0.003]$, confirming statistically significant differences between the employed techniques. The corresponding ranking (Table~\ref{tab:Approach_C_ranking}) identified the best technique within Approach C.
...
\begin{table}[h!]
	...
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{|l|c|c|c|c|c|}
			...
			%% B5 usada como extratora (ver paragrafo apos a Tabela~\ref{tab:CNNArch}). Resultado valido conforme a tese.
			EfficientNetB1+ &  & &  & & \\
			EfficientNetB5 &  & &  & & \\
...
From this initial selection within Approach D, the most relevant technique combinations for this approach were analyzed. The Friedman test resulted in $[X^2(8)=13.266; p<0.103]$, indicating that there are no statistically significant differences between the employed techniques. The corresponding ranking (Table~\ref{tab:Approach_D_ranking}) identified the best-positioned technique within Approach D.
...
Once the most relevant technique combinations for each of the proposed approaches and the reference model were defined, the Friedman test was applied to compare and rank them, yielding $[X^2(4)=3.477; p=0.481]$. As $p>0.05$, there are no statistically significant differences between the studied approaches and the reference model. This result is consistent with hypothesis H1: the proposed approaches classify the images on par with the reference model, without the need for training a convolutional neural network. In Table~\ref{tab:Approaches_friedman_ranking}, the ranked techniques can be observed from the highest to the lowest score, as a result of the Friedman test.
...
Since the omnibus Friedman test was already non-significant ($p=0.481$), the Nemenyi post-hoc procedure is reported here not to detect differences but to quantify and visualise the magnitude of the pairwise gaps relative to the critical difference. For $k=5$ models compared over $N=10$ folds, the critical difference at $\alpha=0.05$ is $CD = q_{\alpha}\sqrt{k(k+1)/(6N)} = 2.728\,\sqrt{30/60} = 1.93$. Even the largest difference between the average Friedman ranks -- Approach C ($3.70$) and Approach D ($2.65$), equal to $1.05$ -- falls well below the critical difference. Therefore, no statistically significant difference was detected between the proposed approaches and the reference model. Fig.~\ref{fig:cd} presents the corresponding critical-difference diagram.

\begin{figure}[h!]
	\centering
	\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}
			% rank axis drawn over the FULL rank range 1..5 (x(r) = (r-1)*3 cm)
			% so the visual spacing of the points reflects their true, small
			% separation relative to the critical difference.
			\draw[thick] (0,0) -- (12,0);
			\foreach \r/\x in {1/0, 2/3, 3/6, 4/9, 5/12}
				\draw (\x,0) -- (\x,0.15) node[above] {\r};
			\node[align=center] at (6,0.95) {Average Friedman rank (higher is better)};
			% critical-difference ruler (length CD = 1.93 ranks = 5.79 cm)
			\draw[|-|] (0,1.55) -- (5.79,1.55) node[midway,above] {CD $= 1.93$};
			% non-significant clique: all five models lie within one CD, so a single
			% bar spans from the lowest-ranked (D, 2.65) to the best (C, 3.70) model
			\draw[line width=3pt] (4.95,-0.18) -- (8.10,-0.18);
			% models -- left-hand labels
			\draw (4.95,0) -- (4.95,-0.8) -- (0.0,-0.8) node[left] {Approach D (2.65)};
			\draw (5.40,0) -- (5.40,-1.3) -- (0.0,-1.3) node[left] {Approach B (2.80)};
			\draw (5.70,0) -- (5.70,-1.8) -- (0.0,-1.8) node[left] {Reference (2.90)};
			% models -- right-hand labels
			\draw (5.85,0) -- (5.85,-0.8) -- (12.0,-0.8) node[right] {Approach A (2.95)};
			\draw (8.10,0) -- (8.10,-1.3) -- (12.0,-1.3) node[right] {Approach C (3.70)};
		\end{tikzpicture}}
	\caption{Critical-difference diagram (Nemenyi post-hoc, $\alpha=0.05$, CD $=1.93$) for the four proposed approaches and the reference model. The horizontal axis spans the full rank range (1--5), and the bracket at the top shows the critical difference to scale: because the entire spread of the five models ($2.65$--$3.70$, a range of only $1.05$) is far smaller than the CD, all of them are linked by a single bar, denoting no statistically significant difference among them.} \label{fig:cd}
\end{figure}

Despite the lack of statistical significance, the reference model imposes a much higher computational burden, as it requires CNN training, whereas the proposed approaches do not involve training a convolutional neural network, only the classifier.
...
In Table~\ref{tab:Kfold_LOOCV_accuracy_comparison}, it is observed that Approach A was the only one to achieve similar performance in both tests, whereas approaches B, C, and D showed a marked reduction in accuracy when the images were tested individually (falling from 0.94--0.96 under 10-fold cross-validation to 0.84--0.87 under LOOCV). This contrast is itself a relevant finding: only the full-feature approach (Approach A) and the fine-tuned reference model remained stable across both validation protocols, indicating that the dimensionality-reduction approaches -- although competitive under 10-fold cross-validation -- do not generalise as reliably at the level of an individual image, a key consideration for a system expected to classify each frame independently.
...
\begin{figure}
	\includegraphics[width=\linewidth]{figHeatmap_no_error.eps}
...
\subsection*{Additional Metrics and Computational Cost of the Recommended Model}

Because accuracy alone is insufficient in a safety-sensitive task, the most stable configuration -- Approach A (EfficientNetB0 + MobileNet with a linear SVM), recommended for deployment -- was further characterized in terms of F1-score and ROC-AUC, computed per fold. Table~\ref{tab:extra_metrics} reports the median values together with the first and third quartiles.

\begin{table}[h!]
	\caption{Additional performance metrics for the recommended configuration (Approach A: EfficientNetB0 + MobileNet + linear SVM), over the 10 folds.}
	\label{tab:extra_metrics}
	\centering
	\begin{tabular}{|l|c|c|c|}
		\hline
		\textbf{Metric} & \textbf{Q1 (25\%)} & \textbf{Median} & \textbf{Q3 (75\%)} \\
		\hline
		Accuracy & 0.9118 & 0.9412 & 0.9706 \\
		F1-score & 0.9231 & 0.9412 & 0.9730 \\
		ROC-AUC  & 0.9118 & 0.9479 & 0.9688 \\
		\hline
	\end{tabular}
\end{table}

The computational cost of this configuration was also measured per fold on the notebook used for the proposed approaches. The one-time feature extraction with the two CNNs took approximately $45.9$~s for the entire $342$-image dataset (about $134$~ms per image), the training of the linear SVM required about $2.8$~s, and classifier inference was approximately $6.7$~ms per image. As the feature extraction relies on lightweight MobileNet-based architectures and the classification stage is computationally trivial, the pipeline is compatible with on-device execution, although smartphone-specific latency and memory footprint remain to be measured.

\subsection*{Limitations and Future Work}
This work has limitations that delimit the scope of its conclusions. First, the evaluation relied on a single, modest-sized dataset (342 images from the VIA dataset); external validation on larger and independent datasets is required to assess generalisation. Second, the best technique of each approach was selected among thousands of combinations, which may optimistically bias the peak 10-fold accuracy (0.9559); the leave-one-out estimate of the most stable configuration (0.9474) should therefore be regarded as the more reliable performance indicator. Third, beyond accuracy, a fuller characterisation in terms of sensitivity and specificity is desirable, given the asymmetric cost of failing to detect an obstacle. Finally, the present study is an offline comparative analysis; the integration of the selected model into a smartphone application and its on-device evaluation -- real-time inference, memory footprint, and a usability assessment with visually impaired users -- constitute the next phase of this research.

\section{Conclusion}
This study explored the use of 25 CNN architectures as feature extractors, combined with dimensionality reduction and supervised classification, for the detection of obstacles in environments navigated by visually impaired individuals. The models, applied without any retraining, showed competitive accuracy compared to fine-tuned alternatives. The best-performing model reached 0.9559 accuracy, and the most stable configuration across tests achieved 0.9474. A Nemenyi post-hoc analysis confirmed that the proposed approaches are statistically equivalent to the fine-tuned reference model, yet at a fraction of its computational cost: once the features have been extracted, classifier inference takes only about 6.7~ms per image. These results demonstrate that systems based exclusively on transfer learning can achieve high performance without significant computational costs. However, limitations were observed in interpreting floor signage, indicating potential for improvement through attention mechanisms or hybrid semantic segmentation. As an offline comparative study, this work establishes the feature-extraction pipeline and the selected model, providing the basis for the on-device deployment and evaluation phase outlined above.

\section*{Code and Data Availability}
The dataset, the source code, and the complete experimental results that support the findings of this study are publicly available at \url{https://github.com/jeffersonarpasserini/Phd-Partial-Results}.
...
\bibitem{who2022}
WHO – World Health Organization. Blindness and vision impairment [Internet], 14 October 2021. World Health Organization (WHO), Geneva, Switzerland, 2018. \url{https://www.who.int/news-room/fact-sheets/detail/blindness-and-visual-impairment}. Last accessed 2 jun. 2022.
...
\bibitem{onu2019}
ONU. World Report on vision. Overview, 8 October 2019. \url{https://www.who.int/publications/i/item/world-report-on-vision}. Last accessed 5 nov 2020. United Nations, Geneva, Switzerland, 2019.
...
\bibitem{islam2019}
Islam, M.M; Sadi, M.S; Zamli, K.Z; Ahmed, M.M. Developing walking assistants for visually impaired people: A review. IEEE Sensors Journal v.19 n. 8, pp. 2814--2828 (2019). \url{https://doi.org/10.1109/JSEN.2018.2890423}.
...
Budrionis, A., Plikynas, D., Daniusis, P., Indrulionis, A.: Smartphone-based computer vision travelling aids for blind and visually impaired individuals: A systematic review. Assistive Technology \textbf{34}(2), pp.178--194 (2022). \url{https://doi.org/10.1080/10400435.2020.1743381}.
...
\bibitem{lin2017}
Lin, B.S., Lee, C.C., Chiang, P.Y.: Simple smartphone-based guiding system for visually impaired people. Sensors \textbf{17}(6), 1371 (2017)
...
\bibitem{mcinnes2021}
McInnes, L.; Healy, J.; Melville, J. UMAP: uniform manifold approximation and projection for dimension reduction. [Internet] September 21, 2020. Disponível em: \url{arxiv.org/abs/1802.03426v3} [stat.ML] 18 Sep 2020. Last accessed 10 nov. 2021
