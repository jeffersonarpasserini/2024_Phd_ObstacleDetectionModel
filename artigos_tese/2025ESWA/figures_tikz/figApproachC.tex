...
\node[vb, draw=cClass, fill=cClassL, below=5mm of c1] (v1){feature vec 1};
\node[vb, draw=cReduce, fill=cReduceL, below=5mm of c2] (v2){feature vec 2};
\node[vw, draw=cReduce, fill=white, below=6mm of im |- v1, yshift=-9mm] (cc){Concatenate $\rightarrow$ reduction};
\node[vw, draw=cReduce, fill=cReduceL, below=of cc] (rv){Reduced vector};
\node[vw, draw=cClass, fill=cClassL, below=of rv] (clf){Supervised classifier};
\node[vw, fill=cOut, text=white, font=\footnotesize\bfseries, below=of clf] (o){Clear / Obstructed};
\draw[vflow](im)--(c1); \draw[vflow](im)--(c2);
\draw[vflow](c1)--(v1); \draw[vflow](c2)--(v2);
...
\draw[vflow](cc)--(rv); \draw[vflow](rv)--(clf); \draw[vflow](clf)--(o);
\end{tikzpicture}
\end{document}
